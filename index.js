...
async function handleRequest(req, res) {
    ...
    if (req.method === 'GET' && parsedUrl.pathname === '/login') {
        await serveLoginPage(res, parsedUrl.query.error);
        return;
    }
    ...
    if (req.method === 'GET' && parsedUrl.pathname === '/register') {
        await serveRegisterPage(res, parsedUrl.query.error);
        return;
    }
        ...
        req.on('end', async () => {
            ...
            if (id && text && text.trim()) {
                try {
                    ...
                    res.end(JSON.stringify({ success: false, error: 'Error updating item' }));
                }
            } else {
